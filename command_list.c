...
// apabila merupakan village, village jadi milik player tersebut ()
{
	...
	if (UnitInfo((P)->currentUnit).movementPoint==0) {
		...
		for (i=UnitInfo((P)->currentUnit).location.X; i<= UnitInfo((P)->currentUnit).location.X + getMovementPoint(UnitInfo((P)->currentUnit)); i++) {
			for (j= UnitInfo((P)->currentUnit).location.Y; j<= UnitInfo((P)->currentUnit).location.Y + getMovementPoint(UnitInfo((P)->currentUnit)); j++) {
				(Po).X=i;
				(Po).Y=j;
				if ((countMove(Po,getUnitLocation(UnitInfo((P)->currentUnit))))<=getMovementPoint(UnitInfo((P)->currentUnit))) {
					if ((M)->MapTable[j][i].unit.type==0)
						(M)->MapTable[j][i].unit.type=5;
				}
			}
			for (j= UnitInfo((P)->currentUnit).location.Y-1; j>= UnitInfo((P)->currentUnit).location.Y - getMovementPoint(UnitInfo((P)->currentUnit)); j--) {
				(Po).X=i;
				(Po).Y=j;
				if ((countMove(Po,getUnitLocation(UnitInfo((P)->currentUnit))))<=getMovementPoint(UnitInfo((P)->currentUnit))) {
					if ((M)->MapTable[j][i].unit.type==0)
	...
	if (canMove) {
		...
		// buildingAddress BA = getBuildingAddressByLocation((M)->villageList,Pos);
...
void NextUnit(Player *P){
	(P)->currentUnit = NextUL((P)->currentUnit);
}
...
// minta input point mana yang bisa diserang
{
	...
	else if (getUnit(M,Ps).type!=0 && getUnit(M,Ps).playerID!=yourUnit.playerID){
		i++;
		optS = i;
		X = getUnit(M,Ps);
		printf("%d. ",i);
	...
	else if (getUnit(M,Pe).type!=0 && getUnit(M,Pe).playerID!=yourUnit.playerID){
		i++;
		optE = i;
		X = getUnit(M,Pe);
		printf("%d. ",i);
	...
	else if (getUnit(M,Pw).type!=0 && getUnit(M,Pw).playerID!=yourUnit.playerID){
		i++;
		optW = i;
		X = getUnit(M,Pw);
		printf("%d. ",i);
...
void whiteMageHealing(Map M, Player *Pl){
	address Pwhite;
	Point Pn,Ps,Pe,Pw;
	Pwhite = searchUnitType((Pl)->unitList, 4);
	if (Pwhite != Nil){
		...
		if (getUnit(M,Pn).type!=0 && getUnit(M,Pn).playerID==yourUnit.playerID ){
			...
			if (Pcurrent!=Nil){
				incHealthWhiteMage(&UnitInfo(Pcurrent), 20);
			}
		}
